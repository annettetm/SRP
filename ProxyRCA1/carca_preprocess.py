...
DATA_DIR = BASE / "data/beauty1_text"
...
def do_general_preprocessing(args, df_rows):
    ...
    item_df = pd.read_csv(PROCESSED / "item_df.csv")
    catalog_iids = (item_df["item_idx"].astype(int) + 1).tolist()

    iid2iindex = {
        iid: index
        for index, iid in enumerate(catalog_iids, start=1)
    }
    ...
    print_timedelta(dt.now() - check) #end
