...
class Carca(nn.Module):
    ...
    def __init__(self,
        ...
        self.use_id = feature_mode in {'id', 'fusion', 'three_way'}
        self.use_content = feature_mode in {'image', 'text', 'fusion'}

        if self.use_content:
            if ifeatures is None:
                raise ValueError(f'ifeatures must not be None for feature_mode={feature_mode}')
        ...
        if self.use_id:
            self.item_id_emb = nn.Embedding(num_items + 1, id_emb_dim)
            self.id_proj = nn.Linear(id_emb_dim, hidden_dim)
        else:
            self.item_id_emb = None
            self.id_proj = None
